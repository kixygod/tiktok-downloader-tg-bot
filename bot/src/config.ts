import { deriveStatsIngestSecret } from "./utils";

export const token = process.env.BOT_TOKEN!;
/** Переопределение из .env или автоматически из BOT_TOKEN (воркер считает так же). */
export const statsInternalToken =
  (process.env.STATS_INTERNAL_TOKEN ?? "").trim() ||
  deriveStatsIngestSecret(token ?? "");
...
export const REDIS_KEY_TTL_SEC = Number(
  process.env.REDIS_KEY_TTL_SEC ?? 30 * 60,
);
...
export const JOBS_RETENTION_DAYS = Number(
  process.env.JOBS_RETENTION_DAYS ?? 10,
);
/** Интервал фоновой очистки jobs (часы) */
export const JOBS_PURGE_INTERVAL_HOURS = Number(
  process.env.JOBS_PURGE_INTERVAL_HOURS ?? 6,
);
